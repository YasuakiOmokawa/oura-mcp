...
- 編集後: `npm run lint:fix` 実行（理由: この project は biome を使う。グローバルルールの `yarn eslint` は eslint を使う project 用で、ここでは適用外。yarn 自体もこの環境にない）
...
- 各 bullet は 30 文字以内（理由: CHANGELOG / リリースノートで一覧された時に読みやすい。詳細は PR 本文に書く）
- リード行も 1 行で完結させる
...
### Claude Code の MCP server 設定ファイル
- User-level: `~/.claude.json` の `mcpServers` セクションを読む
- Project-level: `<project>/.mcp.json`
- `~/.claude/settings.json` / `<project>/.claude/settings.local.json` は permissions / hooks 用で `mcpServers` は読まれない
- 理由: configure ウィザードで間違ったファイルに書くと、整合性チェックは通るのに `/mcp` パネルにサーバが出ない silent failure になる
...
### `gh api` で nested JSON を渡すとき
- `-F dot.path=val` の dot 記法では nested object を確実に組み立てられず 422 が返る
- 回避策: `--input -` + JSON heredoc を使う
...
### `/simplify` 等の agent finding は適用前に裏どり必須
- code-reviewer agent は "Critical" severity でも 30% 程度 false positive を出す（実測: `/simplify` 1 回で 3 つの critical 主張のうち 2 つが誤り）
- 典型的な false positive パターン:
  - **Node.js / framework 内部挙動の誤認**: 例) `process.once` で登録したリスナーは `process.off(event, original)` で解除できないと主張 → 実際は `EventEmitter.removeListener` が `wrapFn.listener === listener` の二段チェックで once-wrapper も解除する
  - **意図的な防御コードの誤検知**: 例) `writeFile({mode})` 後の `chmod` を冗長と主張 → 実際は既存 .tmp ファイル上書き時 `writeFile` の `mode` が無視されるための保険
  - **意図的な UX の誤検知**: 例) `access()` での事前権限チェックを TOCTOU と主張 → 実際は書き込み試行前に明確なメッセージを出すための UX
- 回避策: agent finding を適用する前に
  1. 当該コードを Read で全文確認する
  2. 主張の根拠となる Node.js / framework の挙動を docs / source で裏どりする
  3. 二者が一致してはじめて修正に進む
- 理由: agent は "plausible reasoning" を Critical と断言しがち。鵜呑みにすると正しいコードを退化させる
